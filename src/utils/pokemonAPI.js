  ...
  try {
    ...
    const detailedPokemon = await Promise.all(
      pokemonList.map(async pokemon => {
        ...
        return details.data; // Full Pokémon data
      })
    );

    console.log('Successfully fetched all Pokémon with details:', detailedPokemon);
...
export const getPokemonById = async id => {
  try {
    ...
    throw new Error(`Failed to fetch Pokémon details for ID ${id}. Please try again later.`);
  }
};
...
export const getRandomPokemon = async () => {
  try {
    ...
    throw new Error('Failed to fetch a random Pokémon. Please try again later.');
  }
};
