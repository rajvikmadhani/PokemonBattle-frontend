...
const Battle = () => {
  ...
  const startBattle = () => {
    ...
    const randomCpuPokemon = cpuRoster[Math.floor(Math.random() * cpuRoster.length)];
  ...
  const handlePlayerAttack = isSpecial => {
    ...
    log.push(`${selectedPokemon.name} used ${isSpecial ? 'Special Attack' : 'Attack'}!`);
    ...
    <div className="flex flex-col items-center justify-center h-screen bg-gray-900 text-white">
      ...
      {!isBattleActive && (
        <div className="mb-6">
          ...
          {selectedPokemon && (
            <button onClick={startBattle} className="mt-4 px-6 py-2 bg-green-500 rounded-lg">
              Start Battle
            </button>
          )}
        </div>
            ...
            {cpuPokemon && (
              <div className="absolute top-4 right-4 flex flex-col items-center">
                <img
                  ...
                  className="w-32 h-32"
                />
                <p className="text-lg font-bold capitalize">{cpuPokemon.name}</p>
                <p>HP: {cpuHP}</p>
              </div>
            )}
